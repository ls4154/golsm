...
GOCACHE=/tmp/go-build go run ./cmd/stressrw \
  ...
  -compression=snappy \
  -bloom-bits=10 \
  -max-manifest-file-size=67108864 \
  -max-disk-mb=2048
```
...
- `-oracle`: track expected key/value state in memory; disable for very large working sets.
- `-max-manifest-file-size`: MANIFEST rotation threshold (`0` disables rotation).
