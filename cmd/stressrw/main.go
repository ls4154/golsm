...
func main() {
	...
	fmt.Printf("stressrw: db=%s duration=%s reopen_interval=%s writers=%d readers=%d iterators=%d keyspace=%d value=%d+%d delete=%d%% seek=%d%% writer_delay=%s compression=%s bloom_bits=%d max_file_size=%d max_manifest_file_size=%d max_disk_mb=%d partitioned_writers=%t oracle=%t final_max_mismatches=%d\n",
		cfg.dbPath,
		cfg.duration,
		cfg.reopenInterval,
		cfg.writers,
	...
	if oracle != nil {
		...
		fmt.Printf("stressrw: workload done elapsed=%s oracle=disabled\n", time.Since(start).Round(time.Millisecond))
	}
...
func finalOracleCheck(cfg config, keys [][]byte, oracle *oracleState) error {
	if oracle == nil {
		fmt.Println("stressrw: final oracle check skipped (oracle=false)")
		return nil
	}
	if cfg.reuseDB {
...
func dirSize(path string) (int64, error) {
	...
	err := filepath.WalkDir(root, func(p string, d fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			// Compaction can remove files while we are scanning; skip transient ENOENT
			// for children, but still fail if the root path itself is missing.
...
func parseFlags() config {
	...
	flag.BoolVar(&cfg.oracle, "oracle", true, "track expected key/value state in memory")
	...
	flag.Uint64Var(&cfg.maxManifestSize, "max-manifest-file-size", def.MaxManifestFileSize, "max MANIFEST file size (0: disable rotation)")
