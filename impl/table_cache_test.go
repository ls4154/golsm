...
func buildTableBytes(t *testing.T) []byte {
	...
	f := &memWritableFile{}
	builder := table.NewTableBuilder(f, util.BytewiseComparator, 4096, db.NoCompression, 16)
...
func TestTableCacheEvictionClosesFile(t *testing.T) {
	...
	it1, err := tc.NewIterator(1, uint64(len(data1)))
	...
	it2, err := tc.NewIterator(2, uint64(len(data2)))
	require.NoError(t, err)
	it2.SeekToFirst()
...
func TestTableCacheInUseNotEvicted(t *testing.T) {
	...
	it3, err := tc.NewIterator(3, uint64(len(data3)))
	require.NoError(t, err)
	t.Cleanup(func() {
		if it3 != nil {
			_ = it3.Close()
